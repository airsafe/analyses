...
Below is a table with the actual and estimated percentage of FIO reports by race for each officers group, as well as the expected number of FIO reports for those two groups. 

```{r, echo=FALSE}
chi.table = race.table[,c(1,4,6,7,9)]
chi.table[,2:5] = round(100*chi.table[,2:5]/nrow(fio),1)
colnames(chi.table) = c("Race","Top_quintile", "E(Top_quintile)", "Other_BPD", "E(Other_BPD)")
print.data.frame(chi.table)
```
