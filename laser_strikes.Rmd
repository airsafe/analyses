...
The FAA collected laser encounter data for a total of `r length(state.tot$State)` states and territories. The most encounters in `r ordered.state.tot$State[1]` with `r format(ordered.state.tot$Events[1], big.mark = ",")`. The encounters were concentrated in a few states, with `r sum(state.tot$Events<260)` states and territories having on average fewer than one reported laser encounter per week. The top 10 states in number of laser encounters for 2010 to 2014 were:

1. `r paste(format(ordered.state.tot$Events[1], big.mark = ","),"-",ordered.state.tot$State[1],sep=" ")`
2. `r paste(format(ordered.state.tot$Events[2], big.mark = ","),"-",ordered.state.tot$State[2],sep=" ")`
3. `r paste(format(ordered.state.tot$Events[3], big.mark = ","),"-",ordered.state.tot$State[3],sep=" ")`
4. `r paste(ordered.state.tot$Events[4],"-",ordered.state.tot$State[4],sep=" ")`
5. `r paste(ordered.state.tot$Events[5],"-",ordered.state.tot$State[5],sep=" ")`
6. `r paste(ordered.state.tot$Events[6],"-",ordered.state.tot$State[6],sep=" ")`
7. `r paste(ordered.state.tot$Events[7],"-",ordered.state.tot$State[7],sep=" ")`
8. `r paste(ordered.state.tot$Events[8],"-",ordered.state.tot$State[8],sep=" ")`
9. `r paste(ordered.state.tot$Events[9],"-",ordered.state.tot$State[9],sep=" ")`
10. `r paste(ordered.state.tot$Events[10],"-",ordered.state.tot$State[10],sep=" ")`
...
Air traffic data for the five metropolitan areas was retrieved from the FAA's Air Traffic Activity System (ATADS) located at https://aspm.faa.gov/opsnet/sys/Airport.asp. This resource was used in this report to evaluate the proportion laser encounters for US states or particular us metropolitan areas with the proportion of air traffic.
...
The second one illustrates the percentage of laser encounters by state.
...
The first two maps show that the certain states, for example California, Texas, and Florida, had relativley high proportions of both laser encounters and air traffic compared to many states. 

The third map illustrates the ratio of the percentage of laser encounters divided by the percentage of air traffic activity for each state. The value would be equal to one if the proportion of laser encounters is equal to its proportion of traffic, and greater than one if the proporiton of laser encounters is greater than that state's proporation of air traffic. 

This map is noticably different from the first two graphs, with several states, including Pennsylvania, Delaware, and Oregon, having ratio values that were higher than several states like California and Florida which had a significant fraction of overall US air traffic. As with the previous two maps, darker colors imply higher values, which in this case would be a percentage of laser encounters that is higher than that region's proportion of flight activity.
...
Below are the top 10 states ranked by ratio of percentage of laser encounters divided by percentage of air traffic (risk.ratio variable):
```{r, echo=FALSE}
...
1. `r paste(state.top.10.risk[1,2], " - ", state.top.10.risk[1,1],sep="")`
2. `r paste(state.top.10.risk[2,2], " - ", state.top.10.risk[2,1],sep="")`
3. `r paste(state.top.10.risk[3,2], " - ", state.top.10.risk[3,1],sep="")`
4. `r paste(state.top.10.risk[4,2], " - ", state.top.10.risk[4,1],sep="")`
5. `r paste(state.top.10.risk[5,2], " - ", state.top.10.risk[5,1],sep="")`
6. `r paste(state.top.10.risk[6,2], " - ", state.top.10.risk[6,1],sep="")`
7. `r paste(state.top.10.risk[7,2], " - ", state.top.10.risk[7,1],sep="")`
8. `r paste(state.top.10.risk[8,2], " - ", state.top.10.risk[8,1],sep="")`
9. `r paste(state.top.10.risk[9,2], " - ", state.top.10.risk[9,1],sep="")`
10. `r paste(state.top.10.risk[10,2], " - ", state.top.10.risk[10,1],sep="")`
...
When analyzed on a state level, laser encounters as well as flight operations are not at all distributed evenly, with some states such as California, Texas, and Florida risk of laser encounters appear to be higher in states such as Oregon and Pennsylvania. Specifically, these states account for a higher percentage of laser encounters than they do for total air traffic.

####Potential for missing data####
The data used in this report was compiled by the FAA, and relied on voluntary submissions from pilots, flight crews, and others. While submissions to the database came from a variety of sources, there was no indication in the available FAA literature about whether there were identifiable patterns in the reporting. Specifically, whether there were significant differences in what kind of flight operation or what kind of flying organization was more likely to report laser encounters to the FAA.

One potential issue when is comes to laser exposure data is underreporting. For example, in April 2013, [Michael A. Smith of Omaha, Nebraska was sentenced to 24 months in prison for striking an Omaha Police Department helicopter with a laser on July 11, 2012](https://www.gpo.gov/fdsys/pkg/USCOURTS-ned-8_12-cr-00252/pdf/USCOURTS-ned-8_12-cr-00252-3.pdf). 

While that conviction resulted in both an FBI press release and widespread media attention, the FAA database did not have a full accounting of that event in its laser encounter database, listing the 737 encounter on July  11th, but making no mention of the helicopter encounter. This is particularly noteworthy because [the federal conviction was for the pointing the laser at the police helicopter and not at the airliner](https://www.fbi.gov/omaha/press-releases/2013/omaha-man-given-24-month-prison-sentence-for-pointing-laser-at-aircraft).

It is not clear if this kind of under reporting is common, but if there is to be a greater understanding of the scale and scope of the threats that lasers represent to aircraft, it may be necessary to review a wide range of potential sources of laser encounter reports, for example from military and law enforcement air safety incident reports, in order to develop a more complete picture of the threat posed by lasers. 
...
FAA's Air Traffic Activity System (ATADS) full database  
https://aspm.faa.gov/opsnet/sys/Airport.asp

ATADS summary of all air traffic by state 2010-2014  
http://airsafe.com/analyze/faa_opsnet_by_state.csv
