...
whales = function(df,df.vname){
        ...
        uni.df$Subject = stripper(uni.df$Subject) # Removing leading and trailing spaces
        uni.df$Total = as.numeric(as.character(uni.df$Total))
        uni.df$Cum_subject = 1:nrow(uni.df) # Recall, each row is for one Subject
...
whale.table = function(uni.df){
        ...
        colnames(uni.df.table) = c("Bin_size","Pop") 
        # Pop is the number of times a unique subject had X Occurrences
        uni.df.table$Bin_size = as.numeric(as.character(uni.df.table$Bin_size))
        uni.df.table$Pop = as.numeric(as.character(uni.df.table$Pop))
        uni.df.table$Cum_pop = cumsum(uni.df.table$Pop)
        uni.df.table$Frac_pop = 100*(uni.df.table$Pop/sum(uni.df.table$Pop))
        # # Last column added answers question of how many Occurrences represented by 
        # # the subset of Subjects with 'X' or more occurrences to their credit
        uni.df.table$Tot_fios = (as.numeric(uni.df.table$Bin_size))*(as.numeric(uni.df.table$Pop)) # Number of records for that bin size
        uni.df.table$Cum_fios = cumsum(uni.df.table$Tot_fios) # Cumulative number of FIOs
        uni.df.table$Frac_fios = 100*(uni.df.table$Tot_fios/sum(uni.df.table$Tot_fios))
        uni.df.table$Cdf_pop = cumsum(uni.df.table$Frac_pop)
        uni.df.table$Cdf_fios = cumsum(uni.df.table$Frac_fios)
        return(uni.df.table)
}
...
# To look at race issues, first make Race_id a factor
...
thresh1 = min(which(supercops$Cdf_fios>1)) # minimum number of Subjects to account for 1%+ of FIOs
thresh1.ndx = 1:thresh1

paste("A total of ", thresh1, " officers, representing the bottom ", 
        format(100*length(thresh1.ndx)/nrow(supercops), digits=4),
        "% of officers who submitted FIO reports during the study period,",
        " were responsible for ", format(supercops$Cdf_fios[length(thresh1.ndx)], digits=3),
        "% of the total FIO reports. These officers filed an average of ",
         format(mean(supercops$Total[thresh1.ndx]), digits=3), " FIO reports during this period.",sep="")
...
paste("A total of ", length(top20.ndx), " officers, representing the top ", 
      ...
      " were responsible for ", format(100- supercops$Cdf_fios[top20.ndx[1]-1], digits=4),
      "% of the total FIO reports. These officers filed an average of ",
      format(mean(supercops$Total[top20.ndx]), big.mark = ",", digits = 4), 
...
paste("A total of ", length(top1.ndx), " officers, representing the top ", 
      ...
      format(sum(supercops$Total[top1.ndx]), big.mark = ","), 
      " or ", format(100*sum(supercops$Total[top1.ndx]/nrow(fio)), digits= 4),
      "% of all FIO reports. These officers filed an average of ",
      format(mean(supercops$Total[top1.ndx]), big.mark = ",", digits = 4), 
      " FIO reports during this period.",sep="")
...
paste("A total of ", format(nrow(supercops)-length(extremes.ndx), big.mark = ","), " officers, representing ", 
      ...
      format(sum(supercops$Total[-extremes.ndx]), big.mark = ","), 
      " or ", format(100*sum(supercops$Total[-extremes.ndx]/nrow(fio)), digits= 4),
      "% of all FIO reports. These officers filed an average of ",
      format(mean(supercops$Total[-extremes.ndx]), big.mark = ",", digits = 4), 
      " FIO reports during this period.",sep="")
...
paste("A total of ", length(top20.streets.ndx), " streets, representing the top ", 
      ...
      " were responsible for ", format(100- superstreets$Cdf_fios[top20.streets.ndx[1]-1], digits=4),
      "% of the total FIO reports. These street locations were included an average of ",
      format(mean(superstreets$Total[top20.streets.ndx]), big.mark = ",", digits = 4), 
      " FIO reports during this period.",sep="")
...
paste("A total of ", length(top1.streets.ndx), " streets, representing the top ", 
      ...
      format(nrow(superstreets),big.mark = ","), 
      " street locations included in FIO reports during the study period,",
      " were responsible for ", format(100- superstreets$Cdf_fios[top1.streets.ndx[1]-1], digits=4),
      "% of the total FIO reports. These street locations each accounted for an average of ",
      format(mean(superstreets$Total[top1.streets.ndx]), big.mark = ",", digits = 4), 
      " FIO reports during this period.",sep="")
...
paste("A total of ", length(top20.dates.ndx), " dates, representing the top ", 
      ...
      " were responsible for ", format(100- superdates$Cdf_fios[top20.dates.ndx[1]-1], digits=4),
      "% of the total FIO reports. These dates each accounted for an average of ",
      format(mean(superdates$Total[top20.dates.ndx]), big.mark = ",", digits = 4), 
      " FIO reports.",sep="")
...
paste("A total of ", length(top1.dates.ndx), " dates, representing the top ", 
      ...
      " were responsible for ", format(100- superdates$Cdf_fios[top1.dates.ndx[1]-1], digits=3),
      "% of the total FIO reports. Each of these dates accounted for an average of ",
      format(mean(superdates$Total[top1.dates.ndx]), big.mark = ",", digits = 4), 
      " FIO reports.",sep="")
...
low.per = c(length(thresh1.ndx)/nrow(supercops),sum(supercops$Total[thresh1.ndx])/nrow(fio))
base.per = c((length(extremes.ndx)/nrow(supercops)),sum(supercops$Total[-extremes.ndx])/nrow(fio))
top.quin = c(length(top20.ndx)/nrow(supercops),sum(supercops$Total[top20.ndx])/nrow(fio))
top.one = c(length(top1.ndx)/nrow(supercops),sum(supercops$Total[top1.ndx])/nrow(fio))

perform.comp = 100*(cbind(low.per,base.per,top.quin, top.one))
rownames(perform.comp) = c("People", "Output")
colnames(perform.comp) = c("Lowest", "Baseline", "Top Quintile", "Top 1%")
...
barplot(perform.comp, main="Officer performance by group",
        xlab="Officer group", ylab="Percentage",
        col=c("dodgerblue","coral"), beside=TRUE)

# Legend placed separately
legend(9.5,80, rownames(perform.comp), horiz = FALSE, 
       cex = 0.9,
       fill = c("dodgerblue","coral"))
