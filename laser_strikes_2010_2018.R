...
cat('\n')
paste("Copyright (c) 2019 Todd Curtis, All rights reserved")

cat('\n')
paste("Program created 27 August 2019 and last updated 1 December 2019")
...
cat('\n')
paste("The laser event rates within the top 40 US Metropolitan Statistical Areas (MSAs), were reviewed.")
cat('\n')
paste("This group included the largest US cities, including New York, Chicago, and Los Angeles.")
cat('\n')
paste("Four additional areas, which were major operations hubs for one of the top 10 airlines in laser encounters, were also included.")
cat('\n')
paste("The four additional MSAs were Anchorage, AK; Louisville, KY; Memphis, TN, and Salt Lake City, UT.")
...
# Test for state matching to see if for each state, that
#       city codes with air carrier data are matched to the 
#       right state
# XXY
# Note: unique.state is unique state abbreviations
state.issues = 0
for (i in 1:length(unique.state)){
ww= which(laserhits$State_abb %in% unique.state[i] )
zz = unique(laserhits$Airport[ww])
sss = which(flight.data$Airport %in% zz)
unique(flight.data$State.abb[sss])
if ( length(unique(flight.data$State.abb[sss])) > 1 ){
        state.issues = state.issues +1
        if (state.issues == 1){ 
                cat("\n")
                print("The following states or territories have airport code issues to correct")
                }
        cat(unique.state[i],"airline traffic states","\n")
        print(unique(flight.data$State.abb[sss]))
        print(flight.data$Airport[sss])
        cat("\n")
        print(sss)
        }
}
if (state.issues == 0){ 
        cat("\n")
        paste("No states or territories have airport code issues to correct")
        }
...
cat('\n')
paste("The reviewed MSAs were in the following states and territories:")
...
cat('\n')
paste("The following states and territories were in none of the reviewed MSAs:")
...
cat('\n')
paste("The states and territories that were not a part of any MSA were responsible for ",round(100*excluded.msa.areas.flights/sum(flight.data$Air_carrier_ops), digits = 1),"% of all air carrier flight operations", sep="")
...
paste("The airports in the included MSAs had ", round(100*(sum(flight.data$Air_carrier_ops[msa.flights.ndx]))/sum(flight.data$Air_carrier_ops), digits = 1),"% of all US air carrier traffic.", sep="")
...
paste("The laser events in the reviewed MSAs represented ", round(100*(sum(!is.na(laserhits$Metro_area))/nrow(laserhits)), digits = 1),"% of all US air carrier laser encounters.", sep="")
...
new.areas = state.data[,c("State","State_laser_events","State_air_carrier_ops", "Rate_per_100K" )]
...
cat('\n')
# Now print them out
paste("Combination of MSAs, states, and territories")
combined.areas
